...
Haline shows how ocean warming changes the sea's capacity to dissolve gases, computed from live NOAA data using published solubility science.
...
- Live NOAA global ocean sea surface temperature anomaly
...
Oxygen uses the same Henry's law machinery already in the codebase, so this is new coefficients rather than new architecture. It is also the change that makes the app matter to more people, because oxygen is about animals: reef tanks, dive sites, and fisheries all live or die by it.
